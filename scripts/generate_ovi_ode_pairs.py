...
def main(config, args): 
    ...
    for _, (text_prompt, image_path) in tqdm(enumerate(this_rank_eval_data)):
        ...
        for idx in range(config.get("each_example_n_times", 1)):
            generated_video, generated_audio, generated_image, ode_data_dict = ovi_engine.generate(text_prompt=text_prompt,
            ...
            if sp_rank == 0:
                ...
                torch.save(
                    ode_data_dict,
                    os.path.join(output_dir, f"{filename}.pth")
                )

    ...
    def generate(self,
        ...
        try:
            ...
            if not dist.is_initialized() or dist.get_rank() == 0:
                print(f"Scheduler initialized with {len(scheduler_video.timesteps)} timesteps") 
                print(f"Scheduler sigmas: {scheduler_video.sigmas}")
                print(f"Scheduler timesteps: {scheduler_video.timesteps}")
                print(f"Scheduler timesteps: {scheduler_video.timesteps}")
                print(f"At saving stage, noisy_inputs = noisy_inputs[:, [0, 12, 24, 36, -1]] will be saved, which corresponds to [{', '.join([str(t.item()) for t in scheduler_video.timesteps[[0, 12, 24, 36, -1]]])}]") 
            ...
            if is_i2v and not self.image_model:
                ...
                first_frame = _process_visual(image_path, w=1280, h=704).to(self.device, self.target_dtype)
                # print(f"first frame shape: {first_frame.shape}, device: {self.device}, dtype: {self.target_dtype}")
            else:
            ...
            video_noisy_inputs, audio_noisy_inputs = [], []
            rank = dist.get_rank()
            with torch.amp.autocast('cuda', enabled=self.target_dtype != torch.float32, dtype=self.target_dtype):
                for i, (t_v, t_a) in tqdm(enumerate(zip(timesteps_video, timesteps_audio)), disable=(rank!=0), total=len(timesteps_video), desc="Denoising Steps (CFG Mode)"):
                    timestep_input = torch.full((1,), t_v, device=self.device)
                    video_noisy_inputs.append(video_noise)
                    audio_noisy_inputs.append(audio_noise)
                    if is_i2v:
            ...
            ode_data_dict = {
                ...
                "video_noisy_inputs": torch.stack(video_noisy_inputs, dim=0)[[0, 12, 24, 36, -1], :],
                "audio_noisy_inputs": torch.stack(audio_noisy_inputs, dim=0)[[0, 12, 24, 36, -1], :],
                "first_frame": first_frame,
                "latent_first_frame": latents_images,
                "image_path": image_path,
                # TODO: Whether to save these?
                "pos_forward_args": {
                    'audio_context': [text_embeddings_audio_pos],
                    'vid_context': [text_embeddings_video_pos],
                    'vid_seq_len': max_seq_len_video,
            ...
            return generated_video, generated_audio, image, ode_data_dict

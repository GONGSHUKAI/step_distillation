...
class OviSelfForcingTrainingPipeline(torch.nn.Module):
    ...
    def inference_with_trajectory(
        ...
        **conditional_dict
        ...
        _, mask2 = masks_like(noise_video[:, :self.num_training_frames_video], zero=True)
        mask2 = torch.stack(mask2, dim=0).to(self.device, dtype=dtype) # [B, 4, 48, H, W]
        
        current_start_video, current_start_audio = 0, 0
        ...
        for block_idx in range(self.num_blocks):
            ...
            for i, t_val in enumerate(self.denoising_step_list):
                ...
                if not exit_flag:
                    ...
                    if current_start_video < start_gradient_frame_index_video:
                        ...
                        with torch.no_grad():
                            ...
                            #     current_start=current_start_video * self.frame_seq_length
                            # )
                            x0_v, x0_a, _, _ = self.generator(
                                video_latent=curr_latent_v,
                                audio_latent=curr_latent_a,
            ...
            with torch.no_grad():
                t_context_v = torch.ones((B, self.vid_block_size), device=self.device) * self.context_noise
                t_context_a = torch.ones((B, self.aud_block_size), device=self.device) * self.context_noise

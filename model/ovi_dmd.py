...
class OviDMD(OviSelfForcingModel):
    ...
    def critic_loss(
        ...
        # NOTE: shukai changed this on Dec 27, 2025
        # When doing ovi self forcing, latent_shapes = ([1,32,48,44,80], [1,160,20])
        # But the rollout result (i.e. generated_latents) is of shape ([1,31,48,44,80], [1,157,20])
        # because we did truncation in self._run_generator(...)
        video_shape, audio_shape = generated_video.shape, generated_audio.shape
        batch_size = video_shape[0]
        num_video_frames = video_shape[1]

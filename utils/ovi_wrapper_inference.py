...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"✓ Video shape before VAE: {video.shape}")
        video_latent = vae.encode_video(video)
        print(f"✓ Video latent shape: {video_latent.shape}, dtype: {video_latent.dtype}")
        video_latent = video_latent[:, :2, :, :, :]
        print(f" sliced video latent shape: {video_latent.shape}")
        ...
        print(f"✓ Audio shape before VAE: {audio.shape}")
